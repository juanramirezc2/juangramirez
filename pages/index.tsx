...
const Header = ({ fullpages }: { fullpages: Array<{ text: string, Component: JSX.Element, anchor: string }> }) => {
  return (
...
const skills = [
  ...
  },
  {
    ...
    expertise: '10 Years',
    score: 90,
    icon: <svg version="1.1" viewBox="0 0 256 256">
      <g>
  ...
  },
  {
    ...
    expertise: '5 Years',
    score: 60,
    icon: <svg viewBox="-16.5 0 289 289" version="1.1" >
      <g>
  ...
  return (
    <>
      <Layout background={"linear-gradient(to right, #005AF5, #0081FF)"}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gridTemplateRows: 'repeat(3, 1fr)', width: '100%', columnGap: '10px', rowGap: '10px', height: '80vh', gridAutoRows: '1fr' }}>
          {skills.map((skill) => (
            <div>
              {skill.name}
              {skill.icon}
    ...
    <>
      <div style={{ fontSize: "29px" }} className="percentages">
        <motion.div
          ...
          style={{ textAlign: "left", lineHeight: "50px", padding: "0 1rem", background: '#33BAB3B3' }}
        >
          {Math.floor(progress) + "%"}
        </motion.div>
      </div>
